		...
		<?php if ( ! empty( $tokens ) ) : ?>
			...
			<tr>
				<td colspan="6" align="right">
					<a href="<?php echo home_url() . add_query_arg( 'revoke_all_tokens', '1' ); ?>" class="button-secondary" title="<?php _e( 'Doing this will require the user to login again on all devices.', 'simple-jwt-authentication' ); ?>"><?php _e( 'Revoke all tokens', 'simple-jwt-authentication' ); ?></a>
					<a href="<?php echo home_url() . add_query_arg( 'remove_expired_tokens', '1' ); ?>" class="button-secondary" title="<?php _e( 'Doing this will not affect logged in devices for this user.', 'simple-jwt-authentication' ); ?>"><?php _e( 'Remove all expired tokens', 'simple-jwt-authentication' ); ?></a>
				</td>
			</tr>
		<?php else : ?>
			<tr>
				<td><?php _e( 'No tokens generated.', 'simple-jwt-authentication' ); ?></td>
			</tr>
		<?php endif; ?>
	</tbody>
</table>
